 ...
 * date: 30.11.2010
 */
$DB->query("SELECT test_subject_id FROM tests WHERE test_id={$test_id}");
$DB->fetch_row();
$subject = $DB->record_row[0];
$DB->free_result();
...
$DB->query("SELECT subjects.subject_name 
			FROM subjects, tests 
...
if($DB->get_num_rows() != 0)
{
    while($td_row = $DB->fetch_row($test_details))
    {
        ...
        $question = $DB->query("SELECT * FROM questions WHERE q_test_id='$test_id' AND q_level='$level' ORDER BY question_id ASC");
        $count = 1;
        while($q_row = $DB->fetch_row($question))
        {
            for($k = 0;$k < $DB->get_fields_num();$k++) $q_row[$k] = stripslashes(stripslashes($q_row[$k]));
            echo "<!-- Question #{$count} -->\n";
            echo "<table width=\"100%\" align=\"left\" border=\"0\" cellpadding=\"1\" cellspacing=\"0\" class=\"tbl_view_frame\">\n";
            ...
            ||($q_row[4] == NUMERICAL_IMG)) {
                ...
                echo "<b>{$q_row[2]}</b><br><img src=\"{$source}\" border=\"0\">";
            }
            else {
                //echo "<td align=\"center\" width=\"100%\" colspan=\"2\"><font color=\"Red\"><code><b>{$q_row[2]}</b></code></font></td>\n";
                echo "<td><b>{$count}</b>&nbsp;&nbsp;<b>{$q_row[2]}</b></td>";
            }
            echo "</tr>\n";
            $ans_query = $DB->query("SELECT * FROM answers WHERE aq_id='$q_row[0]' ORDER BY RAND()");
            $ans_counter = 1;
            while($ans_row = $DB->fetch_row($ans_query)) {
                ...
                if($ans_row[2] != "0") {
                    ...
                    echo "<td width=\"80%\" align=\"left\"><img src=\"{$a_media_file}\"><br>{$ans_row[4]}</td>\n";
                }

                else {
                    echo "<td width=\"80%\" align=\"left\">[{$ans_counter}]&nbsp;&nbsp;{$ans_row[4]}</td>\n";
                }
