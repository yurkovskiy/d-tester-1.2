    ...
    &&($_SESSION[$v_name][4] <= MULTI_CHOICE_IMG)) // Radio Button and Check Box questions
{
    ...
    for($i = 0;$i < sizeof($ans_num);$i++) {
        ...
        echo "<input type=\"radio\" id=\"{$fe_id}\" name=\"ans\" class=\"radiobutton\" value=\"{$count_ans}\" onClick=\"is_checked(this)\"><label>{$_SESSION[$ans_name][4]}</label></td>\n";

        if(($_SESSION[$v_name][4] >= MULTI_CHOICE) && ($_SESSION[$v_name][4] <= MULTI_CHOICE_IMG)) // Multi-Choice => CBox
        echo "<input type=\"checkbox\" id=\"{$fe_id}\" name=\"ans[{$i}]\" class=\"radiobutton\" value=\"{$count_ans}\" onClick=\"is_checked(this)\"><label>{$_SESSION[$ans_name][4]}</label></td>\n";
    }
